...
int texturematrix::update(string& data) {
	...
	vector<string> fixedCenters = BZWParser::getValuesByKey("fixedcenter", header, texmatData);
	if(!hasOnlyOne(fixedCenters, "fixedcenter") || !hasNumElements(fixedCenters[0], 2))
		return 0;
	...
	texFixedCenter = TexCoord2D( fixedCenters[0].c_str());
