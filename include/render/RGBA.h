...
public:
	...
	void parseHexFormat(const char* str){
		int bytes[8];
		int index;
		...
		switch (index) {
			...
			case 6: { // rrggbb
				this->set( 
						  (float)((bytes[0] << 4) + bytes[1]) / 255.0f,
						  (float)((bytes[2] << 4) + bytes[3]) / 255.0f,
						  (float)((bytes[4] << 4) + bytes[5]) / 255.0f,
						  1.0f
						  );
			}
			case 8: { // rrggbbaa
				this->set( 
						  ...
						  (float)((bytes[2] << 4) + bytes[3]) / 255.0f,
						  (float)((bytes[4] << 4) + bytes[5]) / 255.0f,
						  (float)((bytes[6] << 4) + bytes[7]) / 255.0f
						  );
			}
		}
	}
	...
	void parseNamedColor(string data, string alpha){
		...
		size_t arraySize = sizeof(colorArray)/sizeof(ColorArrayData);
